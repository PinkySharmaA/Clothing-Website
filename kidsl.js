document.addEventListener('DOMContentLoaded', function() {
  ...
  const addToCartButton = document.getElementById('add-to-cart-button'); // New button
  ...
  addToCartButton.addEventListener('click', function() {
    ...
    const productCost = document.getElementById('product-cost').textContent;
    const productImageSrc = document.getElementById('enlarged-img').src;

    // Add the item to the cart with query parameters
    const queryString = `?name=${encodeURIComponent(productName)}&cost=${encodeURIComponent(productCost)}&src=${encodeURIComponent(productImageSrc)}`;
